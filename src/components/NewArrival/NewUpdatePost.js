...
const NewUpdatePost = ({ id, onClose, onUpdate }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:5000/top-front/front-news/${id}`, {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Send the request to update the post
            await axios.post(`http://localhost:5000/top-front/front-news/update/${id}`, formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
